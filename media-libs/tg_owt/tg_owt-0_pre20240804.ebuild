...
src_prepare() {
	mv -T "${WORKDIR}/libyuv-${LIBYUV_COMMIT}" "src/third_party/libyuv" || die
	mv -T "${WORKDIR}/libsrtp-${LIBSRTP_COMMIT}" "src/third_party/libsrtp" || die

	# The sources for these aren't available, avoid needing them
	sed \
		-e '/include(cmake\/libcrc32c.cmake)/d' \
		-e '/include(cmake\/libabsl.cmake)/d' \
		-i CMakeLists.txt || die

	# "lol" said the scorpion, "lmao" # TODO find better description
	sed '/if (BUILD_SHARED_LIBS)/{n;n;s/WARNING/DEBUG/}' -i CMakeLists.txt || die
