...
RDEPEND="${PYTHON_DEPS}
	...
	media-libs/libpng:=
	>=media-libs/openimageio-3.0.6.1:=[python,${PYTHON_SINGLE_USEDEP}]
	virtual/glu
	virtual/libintl
	virtual/opengl[X?]
	virtual/zlib:=
	...
	color-management? ( media-libs/opencolorio:=[python] )
	...
	optix? (
		>=dev-libs/optix-8:=
		osl? (
			>=media-libs/osl-1.14[clang-cuda,optix,python]
		)
	)
	osl? (
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-4.5.3-cassert.patch"
	"${FILESDIR}/${PN}-4.5.3-gcc-17-include-string.patch"
	"${FILESDIR}/${PN}-9999-don-t-show-variable-names.patch"
	"${FILESDIR}/${PN}-4.5.12-python3.14.patch"
)
...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DWITH_PYTHON_MODULE="no" # $(usex python)" # builds bpy
		...
		-DWITH_CYCLES_LOGGING="$(usex debug)"
...
src_test() {
	...
	if [[ "${RUN_FAILING_TESTS:-0}" -eq 0 ]]; then
		...
		einfo "running failing tests RUN_FAILING_TESTS=${RUN_FAILING_TESTS}"
	fi
...
pkg_postinst() {
	...
	if in_iuse python_single_target_python3_11 && ! use python_single_target_python3_11; then
