...
src_configure() {
	...
	local mycmakeargs=(
		...
		-DWITH_TBB_MALLOC_PROXY="$(usex !tbb)"
...
src_test() {
	...
	if [[ "${RUN_FAILING_TESTS:-0}" -eq 0 ]]; then
		...
		if has_version ">=media-libs/libheif-1.21"; then
			CMAKE_SKIP_TESTS+=(
				"^imbuf_save$"
			)
		fi
	...
	# 81:USE_DEBUG = environ_nonzero("USE_DEBUG")
	local -x USE_DEBUG="$(usex debug "true" "false")"
	"${USE_DEBUG}" && einfo "USE_DEBUG=${USE_DEBUG}"

	# Show the window in the foreground.
	# local -x USE_WINDOW="true" # non-zero
	[[ -v USE_WINDOW ]] && einfo "USE_WINDOW=${USE_WINDOW}"

	# local -x DEBUG="true" # non-zero
	[[ -v DEBUG ]] && einfo "DEBUG=${DEBUG}"
...
pkg_postinst() {
	...
	if in_iuse python_single_target_python3_13 && ! use python_single_target_python3_13; then
