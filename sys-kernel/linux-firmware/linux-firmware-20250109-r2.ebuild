...
inherit dist-kernel-utils linux-info mount-boot multiprocessing python-any-r1 savedconfig
...
src_install() {
	...
	if use savedconfig; then
		if [[ -s "${S}/${PN}.conf" ]]; then
			...
			# grep returns 1 iff no lines, and 2 on error
			grep -v '^#' "${S}/${PN}.conf" 2>/dev/null > "${files_to_keep}"
			ret=$?
			[[ $ret -eq 1 || ! -s "${files_to_keep}" ]] && die "grep failed, empty config file?"
			[[ $ret -eq 2 ]] && die "grep errored"
